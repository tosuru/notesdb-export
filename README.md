...
1. 環境変数：Notesパスワードは安全のため、環境変数`NOTES_PASSWORD`で設定することを推奨します。`PYTHONPATH`は設定必須です。
  　/# Case1:一時的な環境変数設定（現在のセッション内だけ有効にするコマンド例）
   ...
   $env:NOTES_REDIRECT_BASE="http://iszhqw01.isuzu.co.jp/Tools/Redirect.nsf/Index?OpenPage&NotesURL="
   $env:NOTES_DOMAIN_SUFFIX="isuzu.co.jp"
   $env:BOX_SEARCH_BASE="https://isuzu.app.box.com/folder/0/search?query="
   ...
   NOTES_REDIRECT_BASE="http://iszhqw01.isuzu.co.jp/Tools/Redirect.nsf/Index?OpenPage&NotesURL="
   NOTES_DOMAIN_SUFFIX="isuzu.co.jp"
   BOX_SEARCH_BASE="https://isuzu.app.box.com/folder/0/search?query="
   PYTHONPATH="src"
   ```
